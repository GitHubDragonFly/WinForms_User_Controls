...
#Region "Properties"
    ...
    Public Property RI_FixedAspectRatio As Boolean
        ...
        Set(value As Boolean)
            If m_fixedAspect <> value Then
                m_fixedAspect = value
                RotatingImage_Resize(Me, System.EventArgs.Empty)
                Invalidate()
            End If
        End Set
    End Property
    ...
    Public Property RI_RotationAngle As Single
        ...
        End Get
        Set(value As Single)
            If value < -360 OrElse value > 360 Then Exit Property
    ...
    Public Property RI_AnchorPointOfRotation As AnchorPoint
        Get
            Return m_Anchor
        End Get
        Set(value As AnchorPoint)
            If m_Anchor <> value Then
                m_Anchor = value
                Invalidate()
            End If
        End Set
    ...
    Private Sub FlipHV(g As Graphics, img As Bitmap, flipV As Boolean, flipH As Boolean)
        ...
        If flipV Then
            If flipH Then '* Account for horizontal flip
                '* Destination Points: (Width, Height); (0, Height); (Width, 0)
                points = {New Point(Width, Height), New Point(0, Height), New Point(Width, 0)}
            Else
                '* Destination Points: (0, Height); (Width, Height); (0, 0)
                points = {New Point(0, Height), New Point(Width, Height), New Point(0, 0)}
            End If
        End If
